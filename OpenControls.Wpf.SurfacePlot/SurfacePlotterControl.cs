    ...
    public class SurfacePlotControl : OpenTK.GLControl, Model.ILabelFormatter
    {
        ...
        private void CreateTextRenderer()
        {
            _textRenderer = new OpenControls.Wpf.SurfacePlot.TextRenderer.TextRenderer(
                ...
                _iConfiguration.LabelColour,
                _iConfiguration.TransparentLabelBackground ? Color.Transparent : _iConfiguration.BackgroundColour);
            _textRenderer.Load();
        }
        ...
        private Color4 GetVertexColour(float zValue)
        {
            if (_iConfiguration.ShadingAlgorithm == ShadingAlgorithm.ColorBar)
                ...
                return GetVertexColour(zValue, _iConfiguration.ShadingAlgorithm, _iConfiguration.MinimumLevel, _iConfiguration.MaximumLevel);
        }
        ...
        private void DrawShadedSurface(VertexStore vertices)
        {
            ...
            for (int index = 0; index < vertices.Vertices.Length; index += 5)
            {
                ...
                for (int offset = 0; offset < 4; ++offset)
                {
                    ...
                    GL.Vertex3(_axisScaling * vertices.Vertices[indexStart + 1].X,
        ...
        protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
        {
            ...
            GL.ClearColor(_iConfiguration.BackgroundColour);
            GL.Enable(EnableCap.DepthTest);
            ...
            if (_iConfiguration.ShowLabels)
            {
                ...
                if (!_iConfiguration.ViewProjection.IsBirdsEye())
                {
                    ...
                    z = zMin;
                    for (int i = 0; i < _zAxisLabels.Labels.Count; i += 2, z += 2*zGridInc)
                    {
            ...
            switch (configurationItem)
            {
                ...
                case ConfigurationItem.ViewProjection:
                    ...
                    _zAxisLabels.Refresh(
                        ...
                        _textRenderer.MeasureTextLength);
                    break;
                case ConfigurationItem.ShadingAlgorithm:
                    LoadVertices();
                    break;
                case ConfigurationItem.MinimumLevel:
                    LoadVertices();
                    break;
                case ConfigurationItem.MaximumLevel:
                    LoadVertices();
                    break;
                case ConfigurationItem.ColorBar:
                    LoadVertices();
                    break;
            }
            Invalidate(ClientRectangle);
